...
u8 USART_u8ReceiveStringSynchronous(char * Copy_pchString) {

	u32 Local_u32Timeout = 1000;
	u32 Local_u32TimeoutCounter = 0;
	u8 Local_u8Counter = 0;
	u8 Local_u8ReceivedData;
	u8 Local_u8FirstReceiveFlag = 0;
	u8 Local_u8ErrorState = OK;

	if (Copy_pchString != NULL) {

		while (Local_u32TimeoutCounter < Local_u32Timeout) {

			if(Local_u8FirstReceiveFlag == 0) {
			//polling until data is received or timeout reached
			while ((GET_BIT(UCSRA, UCSRA_RXC) == 0) && (Local_u32TimeoutCounter < USART_u32TIMEOUT)) {
				Local_u32TimeoutCounter++;
			}

			} else if(Local_u8FirstReceiveFlag == 1) {
				while ((GET_BIT(UCSRA, UCSRA_RXC) == 0) && (Local_u32TimeoutCounter < Local_u32Timeout)) {
					Local_u32TimeoutCounter++;
				}
			...
			if(GET_BIT(UCSRA, UCSRA_RXC) == 1) {
				...
				Copy_pchString[Local_u8Counter] = Local_u8ReceivedData;
				Local_u32TimeoutCounter = 0 ;
				Local_u8Counter++;
				} else {
					/*Do nothing*/
